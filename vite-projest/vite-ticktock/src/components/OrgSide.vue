...
console.log('org side orgid', props.orgId)
</script>
  ...
  <div class="side fixed w-64 bg-background h-[calc(100%-86.4px)] flex flex-col justify-between">
    <div>
      ...
      <ul class="subject_personal_center text-center">
        <li id="activityList" class="text-xl text-primary bg-gray30 cursor-pointer">
          <a :href="`/org/activityList?orgId=${props.orgId}`" class="block p-5">活動列表</a>
        </li>
        <li id="new_activity" class="text-xl hover:bg-gray30/[35%] cursor-pointer">
          <a :href="`/org/new_activity?orgId=${props.orgId}`" class="block p-5">活動刊登</a>
        </li>
        <li id="orders" class="text-xl hover:bg-gray30/[35%] cursor-pointer">
          <a :href="`/org/orders?orgId=${props.orgId}`" class="block p-5">訂單資訊</a>
        </li>
